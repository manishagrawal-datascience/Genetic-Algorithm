...
def optimize_xgbc(x_train, x_test, y_train, y_test, params={}, error_metric='accuracy_score', population_size=50,
    ...
    try:
        ...
        if 'booster' in params.keys():
            parameters.append(params['booster'])
        else:
            parameters.append(['gbtree'])
...
class optimizer():
    def best_estimator(self, x_train, x_test, y_train, y_test, rf_parameters, error_metric, population_size,
        ...
        population['booster'] = np.random.choice(rf_parameters[3], size=population_size)
        population['reg_alpha'] = np.random.choice(rf_parameters[4], size=population_size)
        population['reg_lambda'] = np.random.choice(rf_parameters[5], size=population_size)
    ...
    def get_mape(self, model, error_metric, x_train, x_test, y_train, y_test):
        ...
        if error_metric == 'accuracy_score':
            error = metrics.accuracy_score(list(y_test), list(predictions))
        elif error_metric == 'f1_score':
            error = metrics.f1_score(list(y_test), list(predictions), average='micro')
        elif error_metric == 'roc_auc_score':
            error = metrics.roc_auc_score(list(y_test), list(predictions), average='micro')
