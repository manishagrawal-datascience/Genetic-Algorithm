...
def optimize_dtc(x_train, x_test, y_train, y_test, params={}, error_metric='accuracy_score', population_size=50,
    ...
    try:
        ...
        if 'criterion' in params.keys():
            ...
            parameters.append(['gini', 'entropy'])
        obj = optimizer()
...
class optimizer:
    ...
    def get_mape(self, model, error_metric, x_train, x_test, y_train, y_test):
        ...
        if error_metric == 'accuracy_score':
            ...
            error = metrics.f1_score(y_test, predictions, average='micro')
            print(error)
        elif error_metric == 'roc_auc_score':
            error = metrics.roc_auc_score(y_test, predictions, average='micro')
